...
//#endregion Switch Accessory

/** Dimmer ACCESSORY CLASS **/
/**
 * [ArduinoDimmerAccessory provides an interface for dimmable devices]
 *
 * @param   {[type]}  sw           [the accessory object]
 * @param   {[type]}  log          [the log object]
 * @param   {[type]}  config       [the platform config object]
 * @param   {[type]}  transceiver  [the transceiver object]
 *
...
function ArduinoDimmerAccessory (sw, log, config, transceiver) {
    ...
    const brightnessCharacteristic = self.service.getCharacteristic(Characteristic.Brightness);
    const onCharacteristic = self.service.getCharacteristic(Characteristic.On);

    brightnessCharacteristic.value = self.currentBrightness;
    
    brightnessCharacteristic
        .on('get', function (cb) {
            self.log(`Get Brightness: ${self.currentBrightness}`);
            cb(null, self.currentBrightness);
    });

    brightnessCharacteristic.on('set', function (value, cb) {
        self.log("Dim");
        self.lastOnTime = Date.now();
    ...
    onCharacteristic.value = self.currentState;
    
    onCharacteristic.on('get', function (cb) {
        cb(null, self.currentState);
    });
    
    onCharacteristic.on('set', function (state, cb) {
        // This "Characteristic.On" "set" event is also fired when
        // Characteristic.Brightness is set.
...
//#endregion Button Accessory

//#region Smoke Accessory
...
//#endregion Smoke Accessory

//#region Water Accessory
...
//#endregion Water Accessory

//#region Motion Dector
...
//#endregion //#region Motion Dector
